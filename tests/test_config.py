...
def test_init_config_force_overwrite(sample_config_content, capture_log, tmp_path, clean_cwd, monkeypatch):
    ...
    config_path = tmp_path / ".prepdir" / "config.yaml"
    config_path.parent.mkdir()
    config_path.write_text(yaml.safe_dump({"OLD_KEY_TO_BE_REPLACED": "old_value_top_be_replaced"}))
    ...
    with config_path.open("r") as f:
        new_config_yaml = yaml.safe_load(f)
    print(f"{new_config_yaml=}")
    show_config_lines(config_path, "new_config")
