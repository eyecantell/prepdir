...
def test_load_config_from_specific_path(sample_config_content, capture_log, tmp_path, clean_cwd):
    ...
    with patch.dict(os.environ, {"PREPDIR_SKIP_CONFIG_LOAD": "false"}):
        config = load_config("prepdir", str(config_path), verbose=2, stdout=stdout_capture, stderr=stderr_capture)
    ...
    assert f"Final config values for UUIDS:\n" in log_output
    stdout_output = stdout_capture.getvalue()
    assert f"Using custom config path: {config_path}" in stdout_output
...
def test_load_config_home(sample_config_content, capture_log, tmp_path, monkeypatch, clean_cwd):
    ...
    stdout_capture = StringIO()
    stderr_capture = StringIO()
    config = load_config("prepdir", verbose=2, stdout=stdout_capture, stderr=stderr_capture)
    ...
    assert f"Loading config with namespace='prepdir'" in log_output
    stdout_output = stdout_capture.getvalue()
    assert f"Found home config: {config_path}" in stdout_output
            ...
            with patch("prepdir.config.tempfile.NamedTemporaryFile", side_effect=PermissionError("Permission denied")):
                with patch.dict(os.environ, {"PREPDIR_SKIP_CONFIG_LOAD": "false"}):
                    with pytest.raises(
                        ...
                        load_config("prepdir", verbose=2, stdout=stdout_capture, stderr=stderr_capture)
    log_output = capture_log.getvalue()
    ...
    with patch("prepdir.config.files", return_value=mock_files):
        with patch("prepdir.config.is_resource", return_value=True):
            with patch.dict(os.environ, {"HOME": str(home_dir), "PREPDIR_SKIP_CONFIG_LOAD": "false"}):
                ...
                config = load_config("prepdir", str(custom_config_path), verbose=2, stdout=stdout_capture, stderr=stderr_capture)
                ...
                assert config.get("VERBOSE") is True
                log_output = capture_log.getvalue()
                assert f"Using custom config path: {custom_config_path}" in log_output
                assert f"Loading config with namespace='prepdir'" in log_output
                ...
                log_output = capture_log.getvalue()
                assert f"Found local config: {local_config_path}" in log_output
                stdout_capture.truncate(0)
                stdout_capture.seek(0)
                stderr_capture.truncate(0)
                stderr_capture.seek(0)
                ...
                assert config.get("VERBOSE") is True
                log_output = capture_log.getvalue()
                assert f"Found home config: {global_config_path}" in log_output
                stdout_capture.truncate(0)
                stdout_capture.seek(0)
                stderr_capture.truncate(0)
                stderr_capture.seek(0)
...
def test_load_config_invalid_yaml(tmp_path, capture_log, clean_cwd):
    ...
    with pytest.raises(ValueError, match="Invalid YAML"):
        load_config("prepdir", str(config_path), verbose=2, stdout=stdout_capture, stderr=stderr_capture)
    log_output = capture_log.getvalue()
    assert f"Using custom config path: {config_path}" in log_output
...
def test_load_config_missing_file(tmp_path, capture_log, clean_cwd):
    ...
    with pytest.raises(ValueError, match=f"Custom config path '{config_path}' does not exist"):
        load_config("prepdir", str(config_path), verbose=2, stdout=stdout_capture, stderr=stderr_capture)
    log_output = capture_log.getvalue()
    assert f"Custom config path '{config_path}' does not exist" in log_output
    stderr_output = stderr_capture.getvalue()
    assert f"Custom config path '{config_path}' does not exist" in stderr_output
    ...
    for namespace in namespaces:
        ...
        with patch("prepdir.config.files", return_value=mock_files):
            with patch("prepdir.config.is_resource", return_value=(namespace == "prepdir")):
                # Test custom config
                config = load_config(namespace, str(tmp_path / f"{namespace}_custom.yaml"), verbose=2, stdout=stdout_capture, stderr=stderr_capture)
                ...
                # Test local config
                config = load_config(namespace, verbose=2, stdout=stdout_capture, stderr=stderr_capture)
...
def test_load_config_verbose_logging(sample_config_content, capture_log, tmp_path, clean_cwd):
    """Test that verbose levels control logging appropriately."""
    config_path = tmp_path / ".prepdir" / "config.yaml"
    config_path.parent.mkdir()
    config_path.write_text(yaml.safe_dump(sample_config_content))
    show_config_lines(config_path)
    ...
    # Test with verbose=0 (no INFO or DEBUG logs)
    logger.setLevel(logging.WARNING)
    with patch.dict(os.environ, {"PREPDIR_SKIP_CONFIG_LOAD": "false"}):
        config = load_config("prepdir", str(config_path), verbose=0, stdout=stdout_capture, stderr=stderr_capture)
    log_output = capture_log.getvalue()
    ...
    # Test with verbose=1 (INFO logs enabled)
    with patch.dict(os.environ, {"PREPDIR_SKIP_CONFIG_LOAD": "false"}):
        config = load_config("prepdir", str(config_path), verbose=1, stdout=stdout_capture, stderr=stderr_capture)
    log_output = capture_log.getvalue()
    assert f"Loading config with namespace='prepdir'" not in log_output
    assert f"Using custom config path: {config_path}" in log_output
    stdout_output = stdout_capture.getvalue()
    ...
    # Test with verbose=2 (DEBUG logs enabled)
    with patch.dict(os.environ, {"PREPDIR_SKIP_CONFIG_LOAD": "false"}):
        config = load_config("prepdir", str(config_path), verbose=2, stdout=stdout_capture, stderr=stderr_capture)
    log_output = capture_log.getvalue()
    assert f"Loading config with namespace='prepdir'" in log_output
...
def test_init_config_existing_file_no_force(sample_config_content, capture_log, tmp_path, clean_cwd):
    ...
    with pytest.raises(SystemExit) as exc_info:
        init_config(namespace="prepdir", config_path=str(config_path), force=False, stdout=stdout_capture, stderr=stderr_capture)
...
def test_init_config_force_overwrite(sample_config_content, capture_log, tmp_path, clean_cwd, monkeypatch):
    ...
    with patch("prepdir.config.files", return_value=mock_files):
        with patch("prepdir.config.is_resource", return_value=True):
            init_config(namespace="prepdir", config_path=str(config_path), force=True, stdout=stdout_capture, stderr=stderr_capture)

    # Verify the config file was overwritten
