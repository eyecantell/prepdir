...
def capture_log():
    ...
    logger.handlers = []
    # Disable propagation to prevent logs going to parent handlers
    logger.propagate = False
    logger.addHandler(handler)
    yield log_stream
    logger.removeHandler(handler)
    # Restore propagation for other tests
...
def test_from_file_path_success(capture_log, tmp_dir):
    """Test successful file reading and UUID scrubbing with verbose and quiet settings."""
    file_path = tmp_dir / "test.txt"
    file_path.write_text("Content with UUID 123e4567-e89b-12d3-a456-426614174000")
    stdout_capture = StringIO()
    stderr_capture = StringIO()

    # Test with verbose=2, quiet=False
    with patch("prepdir.check_stdout_stderr"), patch("prepdir.configure_logging"):
    ...
    assert f"instantiating from {file_path}" in capture_log.getvalue()

def test_from_file_path_binary(capture_log, tmp_dir):
    """Test handling of binary files with verbose and quiet settings."""
    ...
    with patch("prepdir.check_stdout_stderr"), patch("prepdir.configure_logging"):
        entry, uuid_mapping, counter = PrepdirFileEntry.from_file_path(
            ...
            scrub_hyphenated_uuids=True,
            scrub_hyphenless_uuids=False,
            verbose=2,  # Changed from verbose=1 to capture DEBUG log
            quiet=False,
            stdout=stdout_capture,
            stderr=stderr_capture,
        )
    ...
    stdout_output = stdout_capture.getvalue()
    assert "File test.jpg is binary or encoding not supported" in stdout_output
    ...
    assert stdout_capture.getvalue() == ""
    assert "got UnicodeDecodeError with utf-8, presuming binary" not in capture_log.getvalue()  # verbose=1, no DEBUG logs

def test_from_file_path_error(capture_log, tmp_dir):
    """Test handling of file not found with verbose and quiet settings."""
    file_path = tmp_dir / "nonexistent.txt"
    stdout_capture = StringIO()
    stderr_capture = StringIO()

    # Test with verbose=1, quiet=False
    with patch("prepdir.check_stdout_stderr"), patch("prepdir.configure_logging"):
    ...
    assert f"Error: File not found: {file_path}" in stderr_output
    ...
    assert f"File not found: {file_path}" in capture_log.getvalue()
...
def test_from_file_path_read_error(capture_log, tmp_dir):
    ...
    stderr_output = stderr_capture.getvalue()
    assert f"Error: Failed to read {file_path}: Permission denied" in stderr_output
...
def test_from_file_path_empty_file(capture_log, tmp_dir):
    ...
    assert f"instantiating from {file_path}" in log_output
    assert "decoded with utf-8" in log_output
    assert stdout_capture.getvalue() == ""  # No UUIDs to scrub, so no output

def test_from_file_path_invalid_streams(tmp_dir):
    """Test from_file_path with invalid stdout/stderr streams."""
    file_path = tmp_dir / "test.txt"
    file_path.write_text("Sample content")

    # Invalid stdout
    with pytest.raises(ValueError, match="stdout must be a file-like object with a write method"):
        PrepdirFileEntry.from_file_path(
            file_path=file_path,
            base_directory=str(tmp_dir),
            scrub_hyphenated_uuids=False,
    ...
    # Invalid stderr
    with pytest.raises(ValueError, match="stderr must be a file-like object with a write method"):
        PrepdirFileEntry.from_file_path(
            file_path=file_path,
            base_directory=str(tmp_dir),
            scrub_hyphenated_uuids=False,
...
def test_restore_uuids(capture_log, tmp_dir):
    ...
    # Valid mapping with verbose=1, quiet=False
    stdout_capture.truncate(0)
    stdout_capture.seek(0)
    stderr_capture.truncate(0)
    stderr_capture.seek(0)
    ...
    stdout_output = stdout_capture.getvalue()
    assert "Restored UUIDs in test.txt" in stdout_output

    # Invalid mapping with verbose=1, quiet=False
    stdout_capture.truncate(0)
    stdout_capture.seek(0)
    stderr_capture.truncate(0)
    stderr_capture.seek(0)
    ...
    stderr_output = stderr_capture.getvalue()
    assert "Error: No valid uuid_mapping provided for test.txt" in stderr_output
    ...
    assert stderr_capture.getvalue() == ""
    assert f"No valid uuid_mapping provided for test.txt" in capture_log.getvalue()
...
def test_restore_uuids_invalid_streams(tmp_dir):
    """Test restore_uuids with invalid stdout/stderr streams."""
    file_path = tmp_dir / "test.txt"
    file_path.write_text("Content with PREPDIR_UUID_PLACEHOLDER_1")
    entry, _, _ = PrepdirFileEntry.from_file_path(
        file_path=file_path,
...
def test_apply_changes(capture_log, tmp_dir):
    """Test applying changes to a file with verbose and quiet settings."""
    file_path = tmp_dir / "test.txt"
    file_path.write_text("Content with PREPDIR_UUID_PLACEHOLDER_1")
    stdout_capture = StringIO()
    stderr_capture = StringIO()
    ...
    # Successful apply with verbose=1, quiet=False
    stdout_capture.truncate(0)
    stdout_capture.seek(0)
    stderr_capture.truncate(0)
    stderr_capture.seek(0)
    ...
    stdout_output = stdout_capture.getvalue()
    assert "Restored UUIDs in test.txt" in stdout_output
    assert "Applied changes to test.txt" in stdout_output

    # Binary file skip with verbose=1, quiet=False
    ...
    stdout_output = stdout_capture.getvalue()
    assert "Warning: Skipping apply_changes for test.jpg: binary" in stdout_output
...
def test_apply_changes_write_error(capture_log, tmp_dir):
    ...
    stderr_output = stderr_capture.getvalue()
    assert "Error: Failed to apply changes to test.txt: Write error" in stderr_output

def test_apply_changes_invalid_streams(tmp_dir):
    """Test apply_changes with invalid stdout/stderr."""
    file_path = tmp_dir / "test.txt"
    file_path.write_text("Content with PREPDIR_UUID_PLACEHOLDER_1")
    entry, _, _ = PrepdirFileEntry.from_file_path(
        file_path=file_path,
