...
def run(
    ...
    Args:
        ...
        always_show_status: If True, show all messages with is_status=True, regardless of logger level.
    ...
    """
    configure_logging(logger, always_show_status=always_show_status, details=always_show_status)
    ...
    processor = PrepdirProcessor(
        ...
        verbose=always_show_status,  # Pass to processor for consistency
    )
...
def main():
    ...
    parser.add_argument(
        ...
        action="count",
        default=0,
        help="Increase verbosity: -v for INFO and status messages, -vv for DEBUG",
    )
    parser.add_argument(
        "--version",
        action="version",
    ...
    # Set logger level and always_show_status based on verbosity
    if args.verbose >= 2:
    ...
    configure_logging(logger, always_show_status=always_show_status, details=details)
    ...
    try:
        output = run(
            ...
            always_show_status=always_show_status,
            return_raw=False,
        )
        if not args.output:
