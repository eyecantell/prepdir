...
class PrepdirProcessor:
    ...
    def __init__(
        ...
        if not quiet:
            self.logger.info(f"Generated {datetime.now().isoformat()}")
            self.logger.info(f"Traversing directory: {self.directory}")
            self.logger.info(f"Extensions filter: {self.extensions if self.extensions else 'None'}")
        ...
        with redirect_stdout(output):
            ...
            if not files_found:
                if self.specific_files:
                    self.logger.info("No valid or accessible files found from the provided list.")
                    if not self.quiet:
                        self.logger.info("No valid or accessible files found from the provided list.")
                elif self.extensions:
                    self.logger.info(f"No files with extension(s) {', '.join(self.extensions)} found.")
                    if not self.quiet:
                        self.logger.info(f"No files with extension(s) {', '.join(self.extensions)} found.")
                else:
        ...
        for file_path in self.specific_files:
            ...
            try:
                ...
                if not path.exists():
                    ...
                    if not self.quiet and self.logger.getEffectiveLevel() <= logging.INFO:
                        self.logger.info(f"File '{file_path}' does not exist")
                    continue
                if not path.is_file():
                    self.logger.warning(f"'{file_path}' is not a file")
                    if not self.quiet and self.logger.getEffectiveLevel() <= logging.INFO:
                        self.logger.info(f"'{file_path}' is not a file")
                    continue
                ...
                if not self.quiet and self.logger.getEffectiveLevel() <= logging.INFO:
                    self.logger.info(f"Permission denied accessing '{file_path}': {str(e)}")
                continue
    ...
    def _traverse_directory(self) -> Iterator[Path]:
        ...
        try:
            ...
            if not self.quiet and self.logger.getEffectiveLevel() <= logging.INFO:
                self.logger.info(f"Permission denied traversing directory '{self.directory}': {str(e)}")
            return
        ...
        try:
            ...
            for entry in output.files.values():
                ...
                if validate_files_exist and not abs_path.exists():
                    ...
                    if not self.quiet and self.logger.getEffectiveLevel() <= logging.INFO:
                        self.logger.info(f"File {abs_path} does not exist in filesystem")
            return output
    ...
    def init_config(config_path: str = ".prepdir/config.yaml", force: bool = False) -> None:
        ...
        """
        init_config("prepdir", config_path, force)