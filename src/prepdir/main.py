...
def run(
    ...
    Args:
        ...
        verbose: Verbosity level for diagnostic logging (0: INFO, 1: INFO, 2: DEBUG).
    ...
    """
    if not quiet:
        print(f"Starting prepdir in {directory}", file=stdout)
    ...
    if not quiet and not output_file:
        print(output.content, file=stdout)
...
def main():
    ...
    if args.init:
        try:
            PrepdirProcessor.init_config(config_path=args.config, force=False, stdout=sys.stdout, stderr=sys.stderr)
